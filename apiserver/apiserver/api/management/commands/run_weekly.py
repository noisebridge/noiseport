...
class Command(BaseCommand):
    ...
    def send_rate_reminder(self):
        ...
        active_subsidized_members = all_members.filter(
            monthly_fees__gte=30, monthly_fees__lt=50, paused_date__isnull=True
        )

        today = utils.today_alberta_tz()
        _, this_week, _ = today.isocalendar()

        members_to_remind = active_subsidized_members.filter(
            application_date__week=this_week
        )

        self.stdout.write(
            "Found {} members to remind in calendar week {}.".format(
        ...
        for member in members_to_remind:
            ...
            utils.alert_tanner(
                "Subsidized dues reminder {} {}, dues: {}".format(
                    member.user.username,
                    member.user.email,
                    member.monthly_fees,
                )
