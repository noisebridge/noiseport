...
class Command(BaseCommand):
    ...
    def bill_trotec(self):
        ...
        previous_month_start = current_month_start - relativedelta.relativedelta(
            months=1
        )

        self.stdout.write(
            "Billing from {} to {}...".format(
        ...
        month_trotec_user_ids = month_trotec_usages.values_list(
            "user", flat=True
        ).distinct()
        month_trotec_users = User.objects.filter(id__in=month_trotec_user_ids)

        self.stdout.write(
            "Found {} usages by {} users.".format(
        ...
        for user in month_trotec_users:
            ...
            total_seconds = (
                users_usages.aggregate(Sum("num_seconds"))["num_seconds__sum"] or 0
            )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "Completed billing in {} s".format(str(time.time() - start)[:4])
        )
