...
def calc_next_events():
    ...
    # TODO, go by tag?
    member_meeting = (
        sessions.filter(is_cancelled=False, course__in=[317, 413], datetime__gte=now())
        .order_by("datetime")
        .first()
    )
    monthly_clean = sessions.filter(
        is_cancelled=False, course=273, datetime__gte=now()
    ).first()
    next_class = (
        sessions.exclude(course__in=[317, 413, 273])
        .filter(is_cancelled=False, datetime__gte=now())
        .order_by("datetime")
        .first()
    )
    prev_class = (
        sessions.exclude(course__in=[317, 413, 273])
        .filter(is_cancelled=False, datetime__lte=now())
        .order_by("datetime")
        .last()
    )
    ...
    if next_class:
        cache.set(
            "next_class",
            dict(
                datetime=next_class.datetime,
                id=next_class.id,
                name=next_class.course.name,
            ),
        )
    else:
    ...
    if prev_class:
        cache.set(
            "prev_class",
            dict(
                datetime=prev_class.datetime,
                id=prev_class.id,
                name=prev_class.course.name,
            ),
        )
    else:
...
def calc_member_counts():
    ...
    six_month_plus_count = not_paused.filter(
        application_date__lte=six_months_ago
    ).count()
...
def check_minecraft_server():
    if secrets.MINECRAFT:
        ...
        try:
            ...
            logger.error(
                "Problem checking Minecraft: {} - {}".format(
...
def check_mumble_server():
    if secrets.MUMBLE:
        ...
        try:
            ...
            logger.error(
                "Problem checking Mumble: {} - {}".format(e.__class__.__name__, str(e))
            )
...
def check_shopping_list():
    if utils_todo.is_configured():
        try:
            ...
            shopping_list = [
                dict(title=x["title"], created=x["created"]) for x in tasks
            ]
            cache.set("shopping_list", shopping_list)
            return tasks
        except BaseException as e:
            logger.error(
                "Problem checking Shopping List: {} - {}".format(
                    e.__class__.__name__, str(e)
                )
            )
