...
def send_interest_email(interest):
    def replace_fields(text):
        return (
            text.replace(
                "[name]",
                interest.user.member.preferred_name,
            )
            .replace(
                "[course]",
            ...
            .replace(
                "[link]",
                "https://my.protospace.ca/courses/" + str(interest.course.id),
            )
        )
...
def send_overdue_email(member):
    def replace_fields(text):
        return text.replace(
            ...
            "[date]",
            member.expire_date.strftime("%B %d, %Y"),
        )
