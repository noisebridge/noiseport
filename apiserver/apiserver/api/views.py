...
class SearchViewSet(Base, Retrieve):
        ...
        if len(search):
            ...
            if len(results) == 0 and len(search) >= 3 and "@" not in search:
                ...
                fuzzy_results = process.extract(
                    search, choices, limit=20, scorer=fuzz.token_set_ratio
                )
            ...
            if sort == "recently_vetted":
                ...
                queryset = queryset.exclude(
                    is_director=False, is_staff=False, user__is_staff=False
                )
                queryset = queryset.order_by("application_date", "id")
                ...
                if self.request.user.member.allow_last_scanned:
                    ...
                    queryset = (
                        ...
                        .exclude(last_scanned__isnull=True)
                ...
                queryset = (
                    queryset.annotate(
                        pinball_score=Max(
                            "user__scores__score", filter=Q(user__scores__id__gte=6720)
                        ),
                    )
                    .exclude(pinball_score__isnull=True)
                    .order_by("-pinball_score")
                )
            elif sort == "storage":
                queryset = (
                    queryset.annotate(
                    ...
                    .exclude(storage_count=0)
                    .order_by("-storage_count", "id")
                )
            elif sort == "everyone":
    ...
    def discourse(self, request):
        ...
        if (
            secrets.PROTOGRAM_API_TOKEN
            and auth_token != "Bearer " + secrets.PROTOGRAM_API_TOKEN
        ):
        ...
        try:
            ...
            raise exceptions.ValidationError(
                dict(discourse_username="This field is required.")
            )

        member = get_object_or_404(
            models.Member, discourse_username__iexact=discourse_username
        )
...
class MemberViewSet(Base, Retrieve, Update):
    ...
    def pause(self, request, pk=None):
        ...
        msg = "Member has been paused: {} {}".format(
    ...
    def unpause(self, request, pk=None):
        ...
        if difference.days > 370:  # give some leeway
            logging.info(
                "Member has been away for %s days (since %s), unvetting...",
                difference.days,
                member.paused_date,
            )
...
class CourseViewSet(Base, List, Retrieve, Create, Update):
    permission_classes = [
        AllowMetadata | IsAuthenticatedOrReadOnly,
        IsAdminOrReadOnly | IsInstructorOrReadOnly,
    ]
...
class SessionViewSet(Base, List, Retrieve, Create, Update):
    ...
    def get_queryset(self):
        if self.action == "list":
            ...
            return (
                models.Session.objects.annotate(
                    course_count=Count(
                        "course__sessions",
                        filter=Q(
    ...
    def perform_create(self, serializer):
        ...
        if session.datetime < now() + datetime.timedelta(days=1):
            logging.info(
                "Session is in the past or too soon, not sending interest emails."
            )
            return
        ...
        if not unlimited_students and session.course.id != 471 and max_students <= 2:
            logging.info(
                "Session doesn't have enough slots, not sending interest emails."
            )
            return
        ...
        for num, interest in enumerate(interests):
            ...
            if data["request_id"]:
                utils_stats.set_progress(data["request_id"], msg, replace=True)
        ...
        num_satisfied = models.Interest.objects.filter(id__in=interest_ids).update(
            satisfied_by=session
        )

        msg = "Recounting interests..."
        if data["request_id"]:
            utils_stats.set_progress(data["request_id"], msg)
...
class TrainingViewSet(Base, Retrieve, Create, Update):
    ...
    def perform_create(self, serializer):
        ...
        if data.get("member_id", None):
            if not (is_admin_director(user) or session.instructor == user):
                raise exceptions.ValidationError(
                    dict(non_field_errors="Not allowed to register others")
                )
            ...
            if course_id not in [317, 273, 413] and user == session.instructor:
                ...
                raise exceptions.ValidationError(
                    dict(
                        non_field_errors="Can't register the instructor. Don't try to trick the portal."
                    )
                )
            ...
            if training1.exists():
                raise exceptions.ValidationError(
                    dict(non_field_errors="Already registered, refresh the page")
                )
            ...
            if user == session.instructor:
                raise exceptions.ValidationError(
                    dict(non_field_errors="You are teaching this session")
                )
    ...
    def perform_update(self, serializer):
        ...
        if (
            ...
            and not (
                (is_admin_director(user) or session.instructor == user)
                and "student_id" in data
            )
        ):
            raise exceptions.ValidationError(
                dict(
                    non_field_errors="Can't withdraw 24h before class or after. Contact instructor."
                )
            )
    ...
    def refund(self, request, pk=None):
        ...
        if training.paid_date is None:
            raise exceptions.ValidationError(
                dict(
                    non_field_errors="This training has not been paid for yet, so it cannot be refunded."
                )
            )
    ...
    def submit(self, request, pk):
        ...
        if quiz == "sawstop":
            if member.wood_cert_date:
                raise exceptions.ValidationError(
                    dict(non_field_errors="Already certified.")
                )

            # TODO: check if cert requires vetting?

            WOOD1_COURSE = 261
            attended_wood1 = models.Training.objects.filter(
                user=user,
                session__course__id=WOOD1_COURSE,
            ...
            attended_nmo = (
                models.Training.objects.filter(
                    user=user,
                    session__course__id=NMO_COURSE,
                    attendance_status="Attended",
                ).exists()
                or member.orientation_date
            )

            if not attended_nmo:
                raise exceptions.ValidationError(
                    dict(
                        non_field_errors="You haven't attended a New Member Orientation yet."
                    )
                )

            if not attended_wood1:
                raise exceptions.ValidationError(
                    dict(non_field_errors="You haven't attended a Wood I class yet.")
                )
            ...
            if data.dict() != {
                ...
                raise exceptions.ValidationError(
                    dict(non_field_errors="At least one answer is incorrect.")
                )
    ...
    def summary(self, request):
        ...
        for category in [
            ...
            result.append(
                dict(
                    category=category,
                    dollar=txs.filter(category=category).aggregate(Sum("amount"))[
                        "amount__sum"
                    ]
                    or 0,
                    protocoin=-1
                    * (
                        txs.filter(category=category).aggregate(Sum("protocoin"))[
                            "protocoin__sum"
                        ]
                        or 0
                    ),
...
class DoorViewSet(viewsets.ViewSet, List):
    ...
    def seen(self, request, pk=None):
        ...
        logger.info(
            "Scan - Time: {} | Name: {} {} ({})".format(
                t, member.preferred_name, member.last_name, member.id
            )
        )
...
class LockoutViewSet(viewsets.ViewSet, List):
    def list(self, request):
        ...
        for card in cards:
            ...
            authorization["common"] = bool(
                member.orientation_date or member.vetted_date
            )
            authorization["lathe"] = (
                bool(member.lathe_cert_date) and authorization["common"]
            )
            authorization["mill"] = (
                bool(member.mill_cert_date) and authorization["common"]
            )
            authorization["wood"] = (
                bool(member.wood_cert_date) and authorization["common"]
            )
            authorization["wood2"] = (
                bool(member.wood2_cert_date) and authorization["common"]
            )
            authorization["cnc"] = (
                bool(member.tormach_cnc_cert_date) and authorization["common"]
            )
            authorization["tormach_cnc"] = (
                bool(member.tormach_cnc_cert_date) and authorization["common"]
            )
            authorization["precix_cnc"] = (
                bool(member.precix_cnc_cert_date) and authorization["common"]
            )
            authorization["embroidery"] = (
                bool(member.embroidery_cert_date) and authorization["common"]
            )
            authorization["scanner"] = (
                bool(member.scanner_cert_date) and authorization["common"]
            )
    ...
    def authorize(self, request, pk=None):
        ...
        logging.info(
            "Lockout authorization requested by: %s (%s), cert: %s",
            name,
            member.id,
            cert,
        )
...
class StatsViewSet(viewsets.ViewSet, List):
    ...
    def alarm(self, request):
        ...
        auth_token = request.META.get("HTTP_AUTHORIZATION", "")
        if (
            secrets.ALARM_API_TOKEN
            and auth_token != "Bearer " + secrets.ALARM_API_TOKEN
        ):
            raise exceptions.PermissionDenied()
    ...
    def usage(self, request):
        ...
        if should_count:
            start_new_use = (
                not last_use or last_use.finished_at or last_use.username != username
            )
            if start_new_use:
                username_isexpired = (
                    time.time() - track[device]["time"] > 2 * 60 * 60
                )  # two hours
                if username_isfrom_track and username_isexpired:
                    msg = "Usage tracker problem expired username {} for device: {}".format(
                        username, device
                    )
                    # utils.alert_tanner(msg)
                    logger.error(msg)
                ...
                try:
                    ...
                    msg = "Usage tracker problem finding user for username: {}".format(
            ...
            if last_use and not last_use.finished_at:
                ...
                logging.info(
                    "Finishing %s usage #%s, duration: %s",
                    device,
                    last_use.id,
                    duration,
                )
    ...
    def usage_data(self, request):
        ...
        if not (
            is_admin_director(self.request.user)
            or utils.is_request_from_protospace(request)
        ):
            raise exceptions.PermissionDenied()
        ...
        today_total = (
            device_uses.filter(
                ...
                should_bill=True,
            ).aggregate(Sum("num_seconds"))["num_seconds__sum"]
            or 0
        )
    ...
    def scanner3d(self, request, pk=None):
        ...
        if devicename.startswith("raptorx1"):
            ...
            if "card_number" not in request.data:
                raise exceptions.ValidationError(
                    dict(card_number="This field is required.")
                )
...
class UsageViewSet(Base):
    ...
    def csv(self, request):
        ...
        response["Content-Disposition"] = 'attachment; filename="usage-{}.csv"'.format(
            month or "all"
        )
...
class InterestViewSet(Base, Retrieve, Create):
    ...
    def perform_create(self, serializer):
        ...
        interest = models.Interest.objects.filter(
            user=user, course=course, satisfied_by__isnull=True
        )
        if interest.exists():
            raise exceptions.ValidationError(
                dict(non_field_errors="Already interested")
            )

        serializer.save(user=user, satisfied_by=None)
...
class ProtocoinViewSet(Base):
        ...
        try:
            with transaction.atomic():
                ...
                try:
                    ...
                    raise exceptions.ValidationError(
                        dict(balance="This field is required.")
                    )
                except ValueError:
                ...
                try:
                    ...
                    raise exceptions.ValidationError(
                        dict(amount="This field is required.")
                    )
                except ValueError:
                ...
                try:
                    ...
                    raise exceptions.ValidationError(
                        dict(category="This field is required.")
                    )
                ...
                if category == "OnAcct":
                    try:
                        ...
                        raise exceptions.ValidationError(
                            dict(training="This field is required.")
                        )
                    except ValueError:
                        raise exceptions.ValidationError(
                            dict(training="Invalid number.")
                        )
                    ...
                    if not training.session:
                        raise exceptions.ValidationError(
                            dict(training="Invalid session.")
                        )

                    if training.session.is_cancelled:
                        raise exceptions.ValidationError(
                            dict(training="Class is cancelled.")
                        )
                    ...
                    if training.session.cost != amount:
                        ...
                        utils.alert_tanner(msg)
                        raise exceptions.ValidationError(
                            dict(training="Class cost doesn't match amount.")
                        )
                ...
                source_user_balance = (
                    source_user.transactions.aggregate(Sum("protocoin"))[
                        "protocoin__sum"
                    ]
                    or 0
                )
                ...
                if (
                    abs(source_user_balance - balance) > 0.01
                ):  # stupid https://docs.djangoproject.com/en/4.2/ref/databases/#decimal-handling
                    raise exceptions.ValidationError(
                        dict(
                            balance="Incorrect current balance. Try refreshing the page."
                        )
                    )
                ...
                if training:
                    ...
                    tx_memo = "Protocoin - Transaction spent ₱ {} on {}{}".format(
                        amount, category, ", memo: " + memo if memo else ""
                    )
        ...
        try:
            with transaction.atomic():
                ...
                try:
                    ...
                    raise exceptions.ValidationError(
                        dict(member_id="This field is required.")
                    )
                except ValueError:
                ...
                if member_id == source_member.id:
                    raise exceptions.ValidationError(
                        dict(member_id="Unable to send to self.")
                    )
                ...
                memo = "Protocoin - Transaction {} ({}) sent ₱ {} to {} ({})".format(
                    ...
                    source_member.id,
                    amount,
                    destination_member.preferred_name
                    + " "
                    + destination_member.last_name,
    ...
    def get_sessions_teaching(self, user):
        ...
        current_classes = (
            models.Session.objects.exclude(
    ...
    def card_vend_balance(self, request, pk=None):
        ...
        if sessions_teaching:
            ...
            res = dict(balance=5.0, first_name="INSTRUCTOR FREE")
        else:
            user_balance = (
                source_user.transactions.aggregate(Sum("protocoin"))["protocoin__sum"]
                or 0
            )
    ...
    def printer_balance(self, request, pk=None):
        ...
        user_balance = (
            source_user.transactions.aggregate(Sum("protocoin"))["protocoin__sum"] or 0
        )
        user_balance = float(user_balance)
    ...
    def card_vend_request(self, request, pk=None):
        try:
            with transaction.atomic():
                auth_token = request.META.get("HTTP_AUTHORIZATION", "")
                if (
                    secrets.VEND_API_TOKEN
                    and auth_token != "Bearer " + secrets.VEND_API_TOKEN
                ):
                    raise exceptions.PermissionDenied()
                ...
                try:
                    ...
                    raise exceptions.ValidationError(
                        dict(number="This field is required.")
                    )
    ...
    def transactions(self, request):
        transactions = models.Transaction.objects.exclude(protocoin=0).order_by(
            "-date", "-id"
        )
        total_protocoin = (
            transactions.aggregate(Sum("protocoin"))["protocoin__sum"] or 0
        )
    ...
    def printer_report(self, request, pk=None):
        try:
            with transaction.atomic():
                auth_token = request.META.get("HTTP_AUTHORIZATION", "")
                if (
                    secrets.PRINTER_API_TOKEN
                    and auth_token != "Bearer " + secrets.PRINTER_API_TOKEN
                ):
                    raise exceptions.PermissionDenied()
                ...
                logging.info(
                    "New printer job UUID: %s, username: %s",
                    str(job_uuid),
                    str(username),
                )
                ...
                if tx.exists():
                    msg = "Job {}: already billed for in transaction {}, aborting.".format(
                        job_uuid, tx[0].id
                    )
                    utils.alert_tanner(msg)
                    logger.error(msg)
                ...
                if not is_completed:
                    msg = "Job {} user {}: not complete, aborting.".format(
                        job_uuid, username
                    )
                    utils.alert_tanner(msg)
                    logger.error(msg)
                ...
                if not is_print:
                    msg = "Job {} user {}: not a print, aborting.".format(
                        job_uuid, username
                    )
                    utils.alert_tanner(msg)
                    logger.error(msg)
                ...
                cost = squaremetres * PAPER_COSTS.get(
                    paper_name, DEFAULT_PAPER_PROTOCOIN_PER_M
                )
                total_cost += cost
                logging.info(
                    "    %s sqi %s cost: %s", str(squareinches), paper_name, str(cost)
                )
                ...
                if not username:
                    ...
                    try:
                        if (
                            time.time() - track["ARTEMUS"]["time"] < 20 * 60
                        ):  # 20 minutes
                ...
                if not username:
                    msg = "Job {}: missing username, aborting. Cost: {}".format(
                        job_uuid, str(total_cost)
                    )
                    utils.alert_tanner(msg)
                    logger.error(msg)
                    return Response(200)
                ...
                try:
                    ...
                    msg = (
                        "Job {}: unable to find username {}, aborting. Cost: {}".format(
                            job_uuid, username, str(total_cost)
                        )
                    )
                    utils.alert_tanner(msg)
                    logger.error(msg)
...
class PinballViewSet(Base):
    ...
    def score(self, request):
        auth_token = request.META.get("HTTP_AUTHORIZATION", "")
        if (
            secrets.PINBALL_API_TOKEN
            and auth_token != "Bearer " + secrets.PINBALL_API_TOKEN
        ):
            raise exceptions.PermissionDenied()
    ...
    def get_name(self, request, pk=None):
        ...
        def get_favourite_drink(member):
            ...
            memos = list(
                txs.filter(
                    category="Snacks", memo__contains="pop vending machine"
                ).values_list("memo")
            )
...
class HostingViewSet(Base):
    ...
    def offer(self, request):
        auth_token = request.META.get("HTTP_AUTHORIZATION", "")
        if (
            secrets.VANGUARD_API_TOKEN
            and auth_token != "Bearer " + secrets.VANGUARD_API_TOKEN
        ):
            raise exceptions.PermissionDenied()
        ...
        logging.info(
            "Hosting offer from %s %s for %s hours",
            hosting_member.preferred_name,
            hosting_member.last_name,
            hours,
        )

        try:
            current_hosting = models.Hosting.objects.get(
                user=hosting_user, finished_at__gte=now()
            )
            ...
            logging.info(
                "No current hosting for that user, new hosting #%s created.", h.id
            )

            try:  # TODO: remove try / except
                # send a message to Spacebar
                message = (
                    "{} just offered to host for {} hours from now until {}!".format(
                        hosting_user.member.preferred_name,
                        hours,
                        h.finished_at.astimezone(utils.TIMEZONE_CALGARY).strftime(
                            "%-I:%M %p, %b %d"
                        ),
                    )
                )
        ...
        closing = dict(
            ...
            time_str=hosting.finished_at.astimezone(utils.TIMEZONE_CALGARY).strftime(
                "%-I:%M %p"
            ),
    ...
    def high_scores(self, request):
        ...
        members = (
            ...
            .exclude(hosting_hours__isnull=True)
            .order_by("-hosting_hours")
        )
    ...
    def monthly_high_scores(self, request):
        ...
        members = (
            members.annotate(
                hosting_hours=Sum(
                    "user__hosting__hours",
                    filter=Q(user__hosting__finished_at__gte=current_month_start),
                ),
            )
            .exclude(hosting_hours__isnull=True)
            .order_by("-hosting_hours")
    ...
    def high_scores(self, request):
        ...
        members = (
            members.annotate(
                signups=Count(
                    "user__signed_up",
                    filter=Q(user__signed_up__status__in=["Current", "Prepaid"]),
                    distinct=True,
                ),
...
class TodoViewSet(viewsets.ViewSet):
    ...
    def tasks(self, request):
        ...
        try:
            ...
            logger.exception(
                "Problem getting tasks: {} - {}".format(e.__class__.__name__, str(e))
            )
            return Response(
                {"error": str(e)}, status=drfstatus.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def out_of_stock(self, request):
        ...
        try:
            ...
            logger.exception(
                "Problem adding task: {} - {}".format(e.__class__.__name__, str(e))
            )
            return Response(
                {"error": str(e)}, status=drfstatus.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class StorageSpaceViewSet(Base, List, Retrieve, Update):
    ...
    queryset = (
        models.StorageSpace.objects.all().select_related("user__member").order_by("id")
    )
    ...
    def claim(self, request):
        ...
        if storage.location != "member_shelves":
            raise exceptions.ValidationError(
                dict(shelf_id="Not a member shelf. Please see a Director.")
            )

        if storage.user:
            owner_name = storage.user.member.preferred_name
            six_months_ago = utils.today_alberta_tz() - datetime.timedelta(
                days=178
            )  # 2 days slack from the UI
            owner_paused_date = storage.user.member.paused_date
            owner_long_expired = (
                owner_paused_date and owner_paused_date <= six_months_ago
            )

            if not owner_long_expired:
                raise exceptions.ValidationError(
                    dict(shelf_id="Shelf already belongs to {}.".format(owner_name))
                )

        if user.member.paused_date:
            raise exceptions.ValidationError(
                dict(shelf_id="Only active members can claim shelves.")
            )
    ...
    def release(self, request):
        ...
        if storage.user.id != user.id:
            raise exceptions.ValidationError(
                dict(shelf_id="Shelf does not belong to you.")
            )
...
class ToolsViewSet(Base, Create, Destroy):
    ...
    def create(self, request):
        try:
            tool_url = utils_mediawiki.create_tool_page(
                request.data, username=request.user.username
            )
            return Response({"toolUrl": tool_url}, status=drfstatus.HTTP_201_CREATED)
        except Exception as e:
            logger.exception(
                "Create Tool view - {} - {}".format(e.__class__.__name__, str(e))
            )
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.exception(
                "Destroy Tool view - {} - {}".format(e.__class__.__name__, str(e))
            )
            return Response(
                {"error": str(e)}, status=drfstatus.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def categories(self, request):
        ...
        return Response(
            [
                {"value": item.split(" ", 1)[0], "text": item.split(" ", 1)[1]}
...
class OIDCAuthView(views.APIView, AuthorizeView):
    def get(self, request, *args, **kwargs):
        try:
            ...
            logger.exception(
                "OIDC auth view - {} - {}".format(e.__class__.__name__, msg)
            )
            return Response(
                {"error": msg}, status=drfstatus.HTTP_500_INTERNAL_SERVER_ERROR
            )
