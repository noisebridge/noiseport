...
def get_next_tool_id(site):
    ...
    raise Exception(
        "No next tool ID found. Please update the list: https://wiki.protospace.ca/Protospace_Wiki:Wiki-ID_system#Next_available_wiki-ID_numbers"
    )
...
def create_tool_page(form_data, username=None):
    ...
    try:
        ...
        redirect.save(
            "#REDIRECT [[" + name + "]]{{id/after-redirect}}",
            summary="Creating new tool redirect page" + credit,
        )
        rollbacks.append(
            (
                redirect.delete,
                {"reason": "Failed to complete tool creation, initiating rollback"},
            )
        )
        ...
        if "photo" in form_data and form_data["photo"]:
            ...
            site.upload(
                ...
                photo_name,
                1,
                f"Photo of tool {tool_id}",
                comment="Uploaded tool picture" + credit,
            )
...
"""
        ...
        add_to_gallery(
            ...
            name,
            credit=credit,
            new_tool_section=form_copy.get("category", None),
        )
...
def add_to_gallery(
    ...
    photo_name,
    tool_name,
    page_name="Tools_we_have",
    new_tool_section=None,
    credit="",
):
    ...
    gallery_page.save(
        new_text,
        summary=f"Added tool {tool_id} to gallery" + credit,
        section=new_tool_section,
    )
...
def remove_tool_from_gallery(tool_id, page_name="Tools_we_have", credit=""):
    ...
    new_text = "\n".join(
        line for line in gallery_text.splitlines() if f"|link={tool_id}|" not in line
    )
