...
class RegistrationTests(APITestCase):
    ...
    def test_success(self):
        ...
        response = self.client.post(
            self.url, self.data, format="json", HTTP_X_REAL_IP=self.allowed_ip
        )
    ...
    def test_allowed_ip_wrong(self):
        ...
        response = self.client.post(
            self.url, self.data, format="json", HTTP_X_REAL_IP="0.0.0.0"
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    @parameterized.expand(
        [
            (f"{key} is missing", key, status.HTTP_400_BAD_REQUEST)
    ...
    def test_malformed_data(self, name, inp, expected):
        ...
        del copy[inp]
        response = self.client.post(
            self.url, copy, format="json", HTTP_X_REAL_IP=self.allowed_ip
        )
