...
for t in paypal_txs:
    ...
    if not account_id:
        print(
            "Skipping tx id: {}, no payer (could be bank tx)".format(
    ...
    try:
        ...
        print(
            "Unable to find portal transaction for id: {}, ref: {}, date: {}, name: {} {}, email: {}".format(
                t_info["transaction_id"],
                reference,
...
for account_id, member_ids in paypal_accounts.items():
    if len(member_ids) == 0:
        print(
            "Skipping account {}, no members found".format(
                account_id,
            )
        )
        continue
    ...
    if len(set(member_ids)) > 1:
        print(
            "Account {} has multiple members {}, assuming {}".format(
                account_id,
                str(set(member_ids)),
                member_id,
            )
